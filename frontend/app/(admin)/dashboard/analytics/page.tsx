...
const SELLER_NAV = [
  ...
  { label: 'Analytics', href: '/dashboard/analytics', icon: '📊', active: true },
      ...
      <main style={{ marginLeft: sidebarOpen ? 220 : 64, flex: 1, transition: 'margin-left 0.25s', minHeight: '100vh' }}>
        ...
        <div style={{ background: 'var(--white)', borderBottom: '1px solid var(--border)', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 40 }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--serif)', fontSize: '1.4rem', fontWeight: 900, color: 'var(--ink)', letterSpacing: '-0.02em' }}>Analytics</h1>
          </div>
        </div>
        ...
        <div style={{ padding: '4rem 2rem', textAlign: 'center' }}>
          <div style={{ maxWidth: 500, margin: '0 auto' }}>
            ...
            <h2 style={{ fontFamily: 'var(--serif)', fontSize: '1.8rem', fontWeight: 900, color: 'var(--ink)', marginBottom: '0.75rem' }}>Analytics Coming Soon</h2>
            <p style={{ fontSize: '0.95rem', color: 'var(--ink-faint)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
              Advanced analytics with revenue trends, product performance, customer insights, and traffic analysis will be available here once you have order data.
            </p>
            <p style={{ fontSize: '0.85rem', color: 'var(--ink-faint)', background: 'var(--off-white)', padding: '1rem', borderRadius: 4, border: '1px solid var(--border)' }}>
              <strong>For now:</strong> View basic statistics on the <Link href="/dashboard" style={{ color: 'var(--red)', textDecoration: 'none', fontWeight: 500 }}>Dashboard</Link> page
            </p>
          </div>
        </div>
