...
export default function SettingsPage() {
  ...
  // Account state
  const [name, setName]   = useState('Ryan Khanna');
    ...
    <>
              ...
              {SECTIONS.map((s, i) => (
                <button
                  ...
                  style={{
                    ...
                    background: section === s.key ? 'var(--red-light)' : 'transparent',
                    border: 'none',
                    borderLeft: `3px solid ${section === s.key ? 'var(--red)' : 'transparent'}`,
                    borderBottom: i < SECTIONS.length - 1 ? '1px solid var(--border)' : 'none',
                    display: 'flex',
                    alignItems: 'center',
                    gap: '0.65rem',
                    fontSize: '0.84rem',
                    fontWeight: section === s.key ? 500 : 300,
                    color: section === s.key ? 'var(--red)' : 'var(--ink-muted)',
                    cursor: 'pointer',
                    fontFamily: 'var(--sans)',
                    textAlign: 'left',
          ...
          <div>
            ...
            {section === 'notifications' && (
              <>
                ...
                <button onClick={handleSave} style={{ background: 'var(--red)', color: 'var(--white)', border: 'none', borderRadius: 2, padding: '0.75rem 2rem', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--sans)', boxShadow: '0 4px 18px rgba(196,30,58,0.22)' }}>
                  Save preferences
                </button>
              </>
            )}
              ...
              <>
                <Card title="Saved Payment Methods">
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingTop: '1rem' }}>
                    ...
                    <button style={{ padding: '0.75rem', border: '1.5px dashed var(--border)', borderRadius: 3, background: 'none', color: 'var(--ink-muted)', fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'var(--sans)', transition: 'border-color 0.2s, color 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }} onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--red)'; e.currentTarget.style.color = 'var(--red)'; }} onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--ink-muted)'; }}>
              ...
              <>
                ...
                <Card title="Data & Privacy">
                  ...
                  <div style={{ paddingTop: '1rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                    <button style={{ padding: '0.6rem 1.1rem', background: 'none', border: '1px solid var(--border)', borderRadius: 2, fontSize: '0.8rem', color: 'var(--ink-muted)', cursor: 'pointer', fontFamily: 'var(--sans)', transition: 'border-color 0.2s' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--red)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>Download my data</button>
                    <button style={{ padding: '0.6rem 1.1rem', background: 'none', border: '1px solid var(--border)', borderRadius: 2, fontSize: '0.8rem', color: 'var(--ink-muted)', cursor: 'pointer', fontFamily: 'var(--sans)', transition: 'border-color 0.2s' }} onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--red)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>View privacy policy</button>
                  </div>
                </Card>
                <button onClick={handleSave} style={{ background: 'var(--red)', color: 'var(--white)', border: 'none', borderRadius: 2, padding: '0.75rem 2rem', fontSize: '0.88rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--sans)', boxShadow: '0 4px 18px rgba(196,30,58,0.22)' }}>
                  Save settings
            ...
            {section === 'danger' && (
              <div style={{ background: 'var(--white)', border: '1.5px solid #fecaca', borderRadius: 4, padding: '1.75rem' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: 'var(--ink-muted)', fontWeight: 300, marginBottom: '1.75rem', lineHeight: 1.65 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {[
                    ...
                    <div key={item.label} style={{ padding: '1.25rem', background: item.bg, border: `1px solid ${item.color}30`, borderRadius: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                      ...
                      <button style={{ padding: '0.55rem 1.1rem', background: 'none', border: `1.5px solid ${item.color}`, borderRadius: 2, color: item.color, fontSize: '0.8rem', fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--sans)', flexShrink: 0 }}>
                        {item.btnLabel}
                      </button>
                    </div>
                  ))}
      ...
      <style>{`
        @media (max-width: 768px) {
          main > div[style*="grid-template-columns: 220px"] {
